...
The insertion button beside a sidebar text or block field, a table's `emoji` column, and the Emoji tab of the text workspace's character browser all use the same picker. On first use the picker asks for a set before showing its grid. Continue applies that choice to the document; the checked 'Use this set for new work' option also saves a profile seed. A document or link that already chose a set opens the grid directly, and Change emoji set offers the same control again. Cancelling a draft leaves the current style and text alone. Text fields insert at the caret or replace the selection; emoji table cells replace the whole cell. The input's existing edit handlers still own validation, state and history.
