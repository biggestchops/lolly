...
- **Other native text controls.** The sidebar's generic text, long-text and block fields now use the display layer described above. Plain table cells, custom tool editors and editable Design text boxes still use native glyphs while editing; the shared DOM pass cannot insert artwork into a native value or an active contenteditable tree.
