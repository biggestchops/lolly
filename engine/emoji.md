...
Emoji in ordinary text are drawn from a set someone chose, not from the machine. Engine 1.196 wires the foundation below into the render path the shells already use: the runtime walks a rendered tree, replaces each emoji cluster with the chosen pack's canonical artwork, recolours it through a pinned brand treatment and records the pack's rights in the export's Content Credentials. Web, CLI, TUI and MCP run the same code over the same bytes. Nothing falls back to a system emoji font: a set nobody chose, a glyph a set does not carry and a pin the device does not hold all end at a neutral placeholder. The one case that is left alone is a device holding no set at all, where there is nothing to choose and a placeholder would be a mark nobody can clear.

Four things are wired. The pass over every rendered tree and every export, the brand treatment with its own deterministic colour core, three complete packs mounted outside every brand so each profile serves the same files, and the picker grid, which now draws each cell from the chosen set instead of from the machine's font. One thing is not: paragraph layout, because the strict line compiler is still a single LTR specimen and `host.text.layoutRuns` does not exist. "Known gaps in the rendered pass" and "What is not built" below name the rest.
...
`prepareEmojiSvg(pack, meaning, bytes, parseXml)` verifies the artwork digest before invoking the host's XML parser. It returns an opaque prepared handle or an explicit failure. The host supplies a non-networked XML parser; DTD/entity declarations and processing instructions are rejected before parsing. Canonical serialization uses a shared engine tree, not host DOM serialization.

The static subset (`static-svg-v1`) preserves paths, basic shapes, groups, inline presentation attributes, transforms, local linear/radial gradients, `clipPath` elements whose children are shapes or `use` references to a local shape (the idiom Illustrator emits), `clip-path`, `clip-rule` and `paint-order`. Colours must be hex values, CSS colour keywords (canonicalized to hex), `none` or supported local references; inline styles retain precedence over presentation attributes. Declarations that cannot change the picture under this subset are omitted and recorded as changes rather than failing: `enable-background` (no filters), `class` (no stylesheets), `color` (no currentColor paint), `overflow` (nothing establishes a viewport) and `display` values other than `none`. A `display: none` element is dropped and recorded, with any gradient or clip path it contains kept referenceable in a definitions container, as the SVG specification requires. Duplicate ids, dangling references, references of the wrong kind, malformed paths and unsupported attributes fail. The engine rewrites ids and every local reference (paint, clip-path, use href) with a stable placement prefix so repeated glyphs cannot share the wrong gradient or clip. It does not approximate arcs or recover a valid prefix from an incomplete path. Packed arc flags are currently unsupported.

Unsupported visual features include text, images, `use` outside a clip path, masks, filters, external references, CSS stylesheets or variables, animation and foreign namespaces. They cause failure rather than silent removal. Admission bounds include 2 MiB input, 4,096 elements, depth 32, 48 attributes per element, 1 M aggregate attribute characters, 400 K characters per path/list, 100 K numeric arguments, 10 K path segments and finite scalar magnitudes at most 1 M. These are implementation limits, not evidence of acceptable performance for every full family.
...
| Design text boxes | `views/free-canvas/text-edit.ts` reverts before a box becomes editable and redraws after the commit |
...
The rule reads the artwork, not the pack, which is what makes it honest and what makes it reach further than the black set. Measured on the three registered bundles: all 4,316 OpenMoji Black glyphs are single ink, 79 of the 4,315 OpenMoji Color glyphs are (the symbols that set draws as black line art, such as copyright, registered, the white flag and the speech balloons), and none of the 3,953 Twemoji Color glyphs is. Under `influence`, `snap`, `mono` and `duotone` the recipe runs exactly as it always did, because there black is a palette decision like any other colour.
...
The choice travels as two reserved params, on the web and on the CLI alike:

- `emoji=<id>@<version>`, or the last two segments of the id where they name one set the device holds. The checksum is never in the link: the host pins it from its own listing, so a link can name a set and can never describe its bytes.
- `emojifx=original | influence:<bps> | snap | mono | duotone`, with an optional `,unprotected` suffix. The palette comes from the brand in force when the link is opened, never from the URL, so the same treatment reads as the recipient's brand. Which brand colours `mono` and `duotone` take is therefore decided again on every host that opens the link, and it is decided with the treatment's own pinned core rather than the engine's float path, so two hosts pick the same colours.

Unlike `ds` and `designv`, both params are written into share links: the set a document draws is part of the document. A saved session carries the same pair as an `emoji` stamp (session format 3, an additive optional field, older records still open), and `Profile.emoji` holds a personal preference. That preference is a seed and nothing more. It decides what a fresh open starts with, and a document, a session or a link that already names a set keeps the set it names, so turning it on never restyles work that is already made. The order a shell reads is link, then session, then preference.

`parseEmojiParams` and `emojiParams` in [`emoji-style.ts`](src/emoji-style.ts) are the one parser and the one writer, so the web shell, the CLI's `--emoji=` and `--emojifx=` and a session stamp all carry identical bytes. A treatment named with no set is reported as an issue rather than guessed at.

## The pack, and the shared mount that carries it

Three packs are registered, all `tier: on-demand` and all in the one shared root, so every profile offers the same choice. Each is a single `EmojiPackBundleV1` JSON file carrying the exact manifest text plus each glyph's untouched source SVG, so one fetch is a whole set and the boot cost is its index entry:
...
Refused means the engine's static subset would not admit the upstream file: an id with a space in it, mojibake in an id, a stray text node. `--skip-refused` leaves those out and `meta.emoji.missing` names each one with the message it was refused with, so a gap in a set is a record rather than a surprise, and `coverageComplete` is false for both OpenMoji sets because of it. Nothing is withheld from the three registered sets.
...
Noto Emoji Color 2.051 (Apache 2.0) is generated and is NOT registered. Its bundle is 39,062,269 bytes raw (5,274,845 gzip) for 3,678 glyphs, over the 33,554,432 byte ceiling every reader applies (`EMOJI_PACK_MAX_BYTES` in [`emoji-pack.ts`](src/emoji-pack.ts), `BUNDLE_MAX_BYTES` in the web and Node hosts), so registering it would list a set no host would load. Eleven of its files are refused, and two more are withheld on purpose: the rainbow flag `1f3f3-fe0f-200d-1f308` and the package `1f4e6` crash the pinned reference rasteriser (resvg 2.6.2) in their untouched source form and in canonical form, which would take a CLI PNG export down.
...
- **A set chosen while the grid is open redraws it.** The pass is idempotent, so an already-drawn cell would otherwise keep the set it was drawn with. A caller that also hands in `revert` and `onSetChange` gets `run.redraw()`: the characters go back, the probe runs again and the visible cells are drawn from the new set. The popover does not need it (it closes on the pick); the character browser does, and wires it through `runtime.revertEmojiDom` and `runtime.onEmojiChange`.
- **A pass that draws nothing leaves the grid alone.** A probe over a detached copy of the tab strip answers first: with no set chosen, no pack on the device, or no `host.emoji` at all, the watch stops and the component's own grid stands. A wall of several thousand identical placeholders is not a picker. The canvas is where a missing choice is reported and the Emoji section is where it is made.
...
The sidebar's table cells get the same treatment, drawn once when the sidebar builds and again after a pick, so the chrome never shows the machine's glyph either. The `<input>` fields that hold text stay native, as plan 252 allows: a native text input paints its own value, and nothing can place a picture inside one.

## Known gaps in the rendered pass

`applyEmojiToDom` walks HTML text nodes. Three limits are deliberate, and none of
them is a bug report waiting to be filed: they are the shape of what is built.

- **Text inside an `<svg>` root is not drawn from the pack.** `SKIP_TAGS` in
  [`emoji-dom.ts`](src/emoji-dom.ts) skips every `<svg>` subtree, because SVG has
  no way to put a picture inside a text run: a cluster there cannot become a
  nested placement the way it does in HTML, and the honest answers are a separate
  geometry path (resolve each cluster to a nested `<svg>` or `<use>` placed at the
  shaped offset) or a visible refusal. Neither is built. So an emoji typed into a
  tool whose template ROOT is an `<svg>` keeps whatever the machine draws it with,
  and a Tier-A CLI or MCP render of that tool writes the bare code point into the
  file. Measured on this tree, the community tools with an `<svg>` root are
  `asset-export`, `certificate`, `color-palette`, `compress-pdf`, `contrast-check`,
  `convert-image`, `darkroom`, `filter`, `font-convert`, `growth`, `print-sheet`,
  `rebrand-deck`, `redact`, `scan-code`, `signature`, `strip-data`, `timezone` and
  `work-avatar`. `tests/emoji-dom.test.ts` pins the behaviour, so it cannot change
  in either direction without this page changing too.
...
- **Paragraph layout.** The pass places each glyph as an inline element and lets the host lay the line out, which is what makes it work in every tool without a layout engine. The strict compiler (`compileEmojiLine`) is still one LTR line from one font's default instance, with no wrapping, bidi, line breaking or caret mapping, and the shared paragraph contract it was written toward, `host.text.layoutRuns`, does not exist. Nothing in the rendered path depends on it today.
- **A second family on a device.** One family is registered, and one bundle carries it. The other three measured families are evidence from a local import, not something a device can install, and the compact manifest form that would make a 15.7 MB bundle smaller is still a follow-up. Ordered fallback between packs is implemented in the resolver and has no second pack to fall back to.
- **Native text controls.** The pass places a picture inside a text run, which a native `<input>` or `<textarea>` cannot hold: the sidebar's Title field keeps the machine's own glyph while the canvas beside it draws Twemoji. That is the documented allowance, not an oversight, and closing it means replacing those fields with editable elements.
...
- **Readable credits delivery.** The credential records every placed pack; `emojiCreditsText` is written and unused.

The next slice is the compact manifest form that brings the bundle down, then a second family so ordered fallback has something to fall back to, then the optical review that lets `inline-em-v1` normalize between families. Fabrication and spatial representations stay later work, and unsupported recipes are rejected rather than approximated.
