...
**289 modules** under `engine/src/` (excluding the `index.ts` barrel): 212 re-exported from `index.ts`, 215 with a dedicated `tests/*.test.ts`, 66 covered indirectly, 8 with no coverage under `tests/`, 34 wired into the fuzz corpus. Generated by `node scripts/gen-engine-modules.ts` - do not hand-edit between the markers.
...
| `design-lottie.ts` | 148 | Authored Design values into the shared sequence compiler, identical on every host. | no | `tests/design-lottie.test.ts` | – |
...
| `emoji-line.ts` | 169 | Experimental single LTR line master. | no | `tests/emoji-line.test.ts` | – |
...
| `emoji-svg.ts` | 412 | Admit verified artwork to a bounded static SVG subset without silent visual removals. | no | `tests/emoji-svg.test.ts` | yes |
| `emoji-text-path.ts` | 34 | Bounded path sampling for mixed text and emoji on an SVG baseline. | no | indirect | – |
| `emoji-tool-text.ts` | 80 | Mixed vector text for tools that sample artwork or paint their own canvas. | no | indirect | – |
...
| `pdf-map.ts` | 2124 | PDF (and Adobe Illustrator .ai - an .ai IS a PDF) page content stream → DesignNodes. | yes | `tests/pdf-map.test.ts` | yes |
...
| `runtime.ts` | 2265 | Runtime - orchestrates the 5-step lifecycle for a single mounted tool. | yes | indirect | – |
...
| `url-mode.ts` | 1045 | URL mode. | yes | indirect | – |
