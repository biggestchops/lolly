...
**255 modules** under `engine/src/` (excluding the `index.ts` barrel): 208 re-exported from `index.ts`, 201 with a dedicated `tests/*.test.ts`, 48 covered indirectly, 6 with no coverage under `tests/`, 31 wired into the fuzz corpus. Generated by `node scripts/gen-engine-modules.ts` - do not hand-edit between the markers.
...
| `runtime.ts` | 2197 | Runtime - orchestrates the 5-step lifecycle for a single mounted tool. | yes | indirect | – |
...
| `studio3d-camera-path.ts` | 198 | Camera paths: keys captured from the live view, sampled deterministically over the loop. | yes | `tests/studio3d-camera-path.test.ts` | – |
...
| `studio3d-look.ts` | 302 | The boundary between a studio and a document (plan 265 step 2, milestone 2 lane A). | yes | `tests/studio3d-look.test.ts` | – |
| `studio3d.ts` | 564 | Portable studio recipe validation, material finishes and repeatable camera time. | yes | `tests/studio3d.test.ts` | – |
