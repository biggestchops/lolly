...
**259 modules** under `engine/src/` (excluding the `index.ts` barrel): 212 re-exported from `index.ts`, 203 with a dedicated `tests/*.test.ts`, 50 covered indirectly, 6 with no coverage under `tests/`, 31 wired into the fuzz corpus. Generated by `node scripts/gen-engine-modules.ts` - do not hand-edit between the markers.
