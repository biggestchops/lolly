...
**330 modules** under `engine/src/` (excluding the `index.ts` barrel): 236 re-exported from `index.ts`, 236 with a dedicated `tests/*.test.ts`, 76 covered indirectly, 18 with no coverage under `tests/`, 38 wired into the fuzz corpus. Generated by `node scripts/gen-engine-modules.ts` - do not hand-edit between the markers.
...
| `runtime.ts` | 2299 | Runtime - orchestrates the 5-step lifecycle for a single mounted tool. | yes | indirect | – |
...
| `vector-paint-import.ts` | 60 | Lower admitted static artwork into editable contours and a separate paint tree. | yes | indirect | – |
