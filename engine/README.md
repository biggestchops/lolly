...
**256 modules** under `engine/src/` (excluding the `index.ts` barrel): 209 re-exported from `index.ts`, 202 with a dedicated `tests/*.test.ts`, 48 covered indirectly, 6 with no coverage under `tests/`, 31 wired into the fuzz corpus. Generated by `node scripts/gen-engine-modules.ts` - do not hand-edit between the markers.
...
| `geom/boolean.ts` | 1761 | Boolean operations on regions bounded by cubic Béziers - union, intersection, difference, exclusive-or - and the winding-number test they are all decided by. | yes | `tests/geom-boolean.test.ts` | – |
...
| `geom/intersect.ts` | 2054 | Curve intersection. | yes | `tests/geom-intersect.test.ts` | – |
