...
**251 modules** under `engine/src/` (excluding the `index.ts` barrel): 204 re-exported from `index.ts`, 198 with a dedicated `tests/*.test.ts`, 47 covered indirectly, 6 with no coverage under `tests/`, 31 wired into the fuzz corpus. Generated by `node scripts/gen-engine-modules.ts` - do not hand-edit between the markers.
...
| `studio3d.ts` | 392 | Portable studio recipe validation, material finishes and repeatable camera time. | yes | `tests/studio3d.test.ts` | – |
