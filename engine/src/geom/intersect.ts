 ...
 * anything.
 *
 * ## Where clipping cannot see
 *
 * A clip measures how far one curve is from the other's line, not where along it a point
 * is. Two curves that lie along each other within the tolerance (a shape against a copy
 * rounded or rotated by a hair, or two curves that agree to high order where they meet)
 * give it nothing to cut, and bisecting them only doubles the pairs. Three things handle
 * that, in order: `sharedRun` answers curves that are the same curve over a run, a curve
 * against a near-copy of itself is decided from the polynomial of their offset wherever the
 * pieces are regular (`twinNode`, a proof rather than a subdivision), the clip search stops
 * on the remaining pairs it cannot separate (`atResolutionFloor`), and `scanStalled` then
 * finds the crossings and touches in those pairs from the side each point is on. A stopped
 * stretch is decided from outside it: the scan reads the side on each side of the stretch,
 * at the first point where the curves are clearly more than `tol` apart, so a point the clip
 * closed on anywhere inside a zone where the curves agree to within `tol` still brackets the
 * crossing in that zone. Contacts inside a zone are certified by parity and located by
 * distance: the zone's two ends lie outside it, where the side is certain, so ends on
 * opposite sides hold an odd number of crossings and ends on the same side hold a touch or
 * nothing. Any two contacts inside one zone enclose a lens thinner than `tol`, so one
 * contact is the answer at the tolerance asked for.
 *
 ...
 *   well, placed where the gap reaches the tolerance, so a boolean can cut both curves
 *   where the shared run begins and ends.
 ...
 *
 * A near-copy that is parametrised differently (two fits of one arc cut at different places)
 * is not a twin at equal parameters, so `twinNode` does not see it, and such a pair is still
 * searched by clipping; where the copies are a hair apart that costs the clip tree of a
 * near-parallel pair, which doubles at each level until the pieces bend by less than the
 * gap. The boolean recognises those pairs afterwards (`sameTrace`) and decides them alike.
 *
 * On time, measured on a loaded machine with CPU time interleaved against the search before
 * this one (both on the same pairs, same process):
 *
 * - The path this work started from, 185 bytes of repeated coincident C/S curve pairs:
 *   selfUnion 40 ms, union 30, difference 52, stroke 89, against 1660 to 1800 ms for
 *   selfUnion, union and difference and 860 for stroke.
 * - Two near-coincident loops traced in opposite directions, a ten-millionth apart: 1 to
 *   13 ms a pair, against 50 to 1900 ms; the 44 tangent pairs 17 ms in total against 110 s.
 * - The 240 near-copy pairs at cusps, loops and hairpins: 1.05 s in total against 49.9 s,
 *   worst 88 ms against 5133; 12 are more than twice as slow as old and over 10 ms, every
 *   one a pair old answered in under 10 ms with a scatter.
 *
...
export function cubicRoots01(a: number, b: number, c: number, d: number, dirs?: number[]): number[] {
  ...
  const push = (t: number, dir: number) => {
    if (t >= -T_EPS && t <= 1 + T_EPS) { out.push(Math.min(1, Math.max(0, t))); sg.push(dir); }
  };
  const f = (t: number) => ((a * t + b) * t + c) * t + d;
  const df = (t: number) => (3 * a * t + 2 * b) * t + c;
  ...
  // The interval is widened by the parameter slack every root is accepted at: a curve
  // starting exactly on a line has its root a rounding error outside [0, 1], at -1e-19, and
  // isolating over [0, 1] alone found no sign change there. Every end cap of a band against
  // the curve it caps lost its vertex that way.
  ...
  if (Math.abs(qa) > 1e-300) {
    ...
    if (disc >= 0) {
      ...
      for (const r of q !== 0 ? [q / qa, qc / q] : [-qb / (2 * qa)]) if (r > -T_EPS && r < 1 + T_EPS) cuts.push(r);
    }
  } else if (Math.abs(qb) > 1e-300) {
    const r = -qc / qb;
    if (r > -T_EPS && r < 1 + T_EPS) cuts.push(r);
  }
  cuts.sort((x, y) => x - y);
...
function dedupeRoots(ts: number[], sg: number[], dirs?: number[]): number[] {
  ...
  if (dirs) { dirs.length = 0; dirs.push(...dd); }
  return out;
}
 ...
 */
function clipToFatLine(c: Cubic, fat: NonNullable<ReturnType<typeof fatLine>>, pad = 1e-12): [number, number] | null {
  const d = [
  ...
  // The hull chains are cut at the PADDED levels, as the endpoints are tested against them:
  // cut at the bare band, the surviving range at depth was decided by distances a few
  // hundred ulps wide against a band a few ulps wide, and two arcs a hair apart lost their
  // crossing to a range that no longer held it. A range kept on the padded band is at worst
  // a little wider than it need be, and the search bisects or stalls it as any other.
 ...
 *  placed on and still be kept for the scan. See the point branch of `clipIntersect`. */
...
function clipIntersect(
  ...
  const clipped = fat ? clipToFatLine(c1, fat, search.pad) : [0, 1] as [number, number];
  if (!clipped) return;
  const [lo, hi] = clipped;
  const shrink = hi - lo;
  ...
  if (shrink > 0.8) {
    ...
    if (s1 >= s2) {
      ...
      clipIntersect(a, c2, t1lo, mid, t2lo, t2hi, tol, depth + 1, search, swap);
      clipIntersect(b, c2, mid, t1hi, t2lo, t2hi, tol, depth + 1, search, swap);
    } else {
      const [a, b] = splitCubic(c2, 0.5);
      const mid = (t2lo + t2hi) / 2;
      clipIntersect(c1, a, t1lo, t1hi, t2lo, mid, tol, depth + 1, search, swap);
      clipIntersect(c1, b, t1lo, t1hi, mid, t2hi, tol, depth + 1, search, swap);
    }
    return;
  }
...
export function intersectCubics(c1: Cubic, c2: Cubic, tol = EPS): Intersection[] {
  ...
  const search: ClipSearch = { c1, c2, out: [], stalled: [], pad: Math.max(1e-12, mag * 64 * Number.EPSILON), size, twin, c1r, c2r };
