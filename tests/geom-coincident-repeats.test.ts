 ...
 * second (stroke about half a second).
 *
 * The fix is a work budget. The clip search counts its recursion nodes, and a pair that
 * spends more than `CLIP_BUDGET.maxNodes` of them is abandoned and answered instead by the
 * overrun search in engine/src/geom/intersect.ts, which stops on pieces it cannot separate
 * (`atResolutionFloor`), finds any crossing in them from which side of one curve the other is
 * on (`scanStalled`), and answers curves that share a stretch of boundary with the two ends
 * of that stretch before any clipping at all (`sharedRun`). About 25 pairs of the 784 this
 * path produces per operation go over; every other pair in the tree keeps the clip search's
 * own answer, which is what tests/geom-clip-budget.test.ts pins. Stopping without the scan,
 * the first version of this work, lost real shallow crossings; those cases are in
 * tests/geom-shallow-crossings.test.ts.
 *
 * ## Oracles
 *
