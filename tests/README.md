...
- `tests/helpers/` - shared non-test helpers (`photo-like.ts`, the calibrated pixel-watermark content generator; `host.ts`, the minimal stub host for tool-contract suites). The glob only collects `*.test.ts`, so these are never run as tests; `tests/tsconfig.json`'s `./**/*` include still typechecks them. The 3D Studio suites share four: `studio3d-browser.ts` (the Playwright harness every `studio3d*.browser.test.ts` file uses: it bundles the studio mount module, serves the 3d-studio template and the fixture routes, and holds the encode tier so studio browser files run one after another), `studio3d-dom.ts` (a jsdom `DOMParser` and `XMLSerializer` for the Node source loader, plus `inlineClassFills`, because jsdom does not apply `<style>` rules to an XML document), `studio3d-glb.ts` (a deterministic three-material textured GLB for the material override checks) and `studio3d-alpha.ts` (the cutout checks: clear outside, the shadow bound and no halo).
- `tests/fixtures/studio3d/` - 3D Studio evidence from plan 265. `recipes/` pins what the pushed 3D Studio 0.4.0 (commit `05faef7a4`) evaluates: one value set and scene per file, plus `materials.json`; `generate.ts` wrote them and refuses to run once the studio code differs from that commit, because the files are a record, not something to regenerate. `geometry/` holds the SVG bevel and hole fixtures and `tessellation-baseline.json`. `lighting/backends.json` keeps the lighting measures per backend, each entry marked reviewed or not, and `alpha/flat-backdrop.json` records what a flat backdrop colour becomes on each backend.
...
- **Mobile web:** `LOLLY_MOBILE_TEST_URL=http://127.0.0.1:5173 LOLLY_MOBILE_TEST_BROWSER=webkit node --test tests/mobile-web.browser.test.ts` checks phone and tablet viewport scaling, touch authoring, quiz creation, export preflight, ZIP download and learner progress. Use `chromium` for the second engine. Temporary persistent profiles exercise file storage; screenshots go to `/tmp/lolly-mobile-web` or `LOLLY_MOBILE_TEST_OUTPUT`. Run against a served release build before deployment too, since HTML build transforms can change mobile scaling. These browser tests complement Safari device or simulator checks.
...
- **3D Studio:** each `studio3d*.browser.test.ts` file skips its whole `describe` when Playwright Chromium is missing (`pnpm exec playwright install chromium`). The pushed renderer comparison in `studio3d-compat.browser.test.ts` needs commit `05faef7a4`, and one case in `studio3d-recipe-compat.test.ts` reads the 0.3.0 manifest from commit `d7b4593dd`, so both skip on a shallow clone such as CI's. Switches: `STUDIO_SHELL_URL` (a running web shell) enables `studio3d.shell.browser.test.ts`; `STUDIO_SUSE=1` enables the private SUSE fixtures (it needs `brands/suse`); `STUDIO_NATIVE=1` renders on the Metal ANGLE backend instead of SwiftShader; `STUDIO_SHOTS=<dir>` saves every render for review; `STUDIO_COMPAT_SELF=1` makes the compatibility suite compare the pushed renderer with a second copy of itself, which shows whether a backend renders the pinned sets repeatably; `STUDIO_WRITE_BASELINE=1` records the measures of the current backend, the lighting measures as unreviewed entries in `fixtures/studio3d/lighting/backends.json` (a run never marks an entry reviewed and never overwrites a reviewed one) and the flat backdrop colour in `fixtures/studio3d/alpha/flat-backdrop.json`.
