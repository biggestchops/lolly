...
export function blockMarkup(
  ...
  const fields =
    block.kind === 'text'
      ...
      ${['image', 'slides'].includes(block.kind) ? `<label class="learning-check"><input type="checkbox" data-block-field="decorative" ${block.decorative ? 'checked' : ''}><span>Decorative image<small>No explanation is needed for this image.</small></span></label>` : ''}
