...
import { assetDependency, decodeAssetVersion, encodeAssetVersion } from '../../../../engine/src/asset-version.ts';
...
export function rebaseImportedAssetPins<T>(data: T, rekey: ReadonlyMap<string, string>, records: readonly BeamAssetRecord[]): T {
  ...
  const walk = (value: unknown, depth: number): unknown => {
    ...
    return Object.fromEntries(Object.entries(ref).map(([key, item]) => [key, key === 'textDocument' ? mapTextFontAssets(item, id => {
      const dep = assetDependency({ id }); if (!dep.pin || !received.has(dep.id)) return id;
      const record = versions.get(dep.id); if (!record?.version) throw new Error('This shell cannot retain the imported text font version.');
      return encodeAssetVersion(dep.id + dep.modifier, { version:record.version, format:record.format });
    }) : walk(item, depth + 1)]));
  };
