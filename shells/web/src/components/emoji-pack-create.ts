...
export function mountEmojiPackCreate(root: HTMLElement, api: EmojiAPI, installed: (info: EmojiSetInfoV1) => void): void {
  ...
  async function open(): Promise<void> {
    ...
    const modal = mountModal(`<form class="field-stack" style="max-width:36rem;padding:1rem">
      <h2>${t('Create emoji set')}</h2>
      ...
      <label class="field-row">${t('SVG artwork')}<input name="art" type="file" accept=".svg,image/svg+xml" multiple required></label>
      <p role="status" data-status></p><button type="submit">${t('Create set')}</button><button type="button" data-cancel>${t('Cancel')}</button>
    ...
    modal.el.querySelector('form')!.addEventListener('submit', event => {
      ...
      const status = form.querySelector<HTMLElement>('[data-status]')!; status.textContent = tRaw('Checking emoji artwork…');
      void (async () => {
        ...
        const files = Array.from(form.querySelector<HTMLInputElement>('[name=art]')!.files ?? []);
