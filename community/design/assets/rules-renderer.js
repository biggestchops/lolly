"use strict";var LollyDesignRules=(()=>{var S=Object.defineProperty;var N=Object.getOwnPropertyDescriptor;var A=Object.getOwnPropertyNames;var $=Object.prototype.hasOwnProperty;var F=(n,p)=>{for(var l in p)S(n,l,{get:p[l],enumerable:!0})},j=(n,p,l,o)=>{if(p&&typeof p=="object"||typeof p=="function")for(let u of A(p))!$.call(n,u)&&u!==l&&S(n,u,{get:()=>p[u],enumerable:!(o=N(p,u))||o.enumerable});return n};var C=n=>j(S({},"__esModule",{value:!0}),n);var L={};F(L,{designSelection:()=>k,designToolPolicy:()=>T,evaluateDesignTool:()=>_,validateDesignTool:()=>P,validateDesignValues:()=>D});var R={text:["text","longtext","select"],image:["asset"],fontSize:["number"],font:["select"],weight:["select","number"],fg:["color","select"],fill:["color","select"],fit:["select"],imageFraming:["vector"]},z=new Set(["__proto__","prototype","constructor"]),M=new Set(["boxes","customCss","background","transparentBg","font","fontSize","boxStyle","textStyle","mediaHtml","textHtml","designRows","designWidth","designHeight","designBackground","designIssues"]),O=/^[a-zA-Z][a-zA-Z0-9_-]{0,63}$/,v=(n,p)=>Object.hasOwn(n,p),w=n=>`${n.variantId}/${n.layerId}/${n.property}`;function T(n){return{schemaVersion:1,presentation:n.presentation,inputs:n.inputs,choices:n.choices,variants:n.variants.map(({id:p,label:l,width:o,height:u})=>({id:p,label:l,width:o,height:u})),defaultVariant:n.defaultVariant,formats:n.formats,...n.sourceTool?{sourceTool:n.sourceTool}:{}}}function P(n,p=[]){let l=[],o=(e,i,t,s)=>{l.push({code:e,message:i,inputId:t,layerId:s})};n.schemaVersion!==1&&o("version","This rules version is not supported."),/^[a-z0-9][a-z0-9-]{2,95}$/.test(n.id)||o("identity","Use a valid permanent tool id."),(!n.name.trim()||!/^\d+\.\d+\.\d+$/.test(n.version))&&o("identity","Add a name and a three-part version."),["sidebar","on-canvas"].includes(n.presentation)||o("presentation","Choose Sidebar or On-canvas."),(!n.formats.length||n.formats.some(e=>!["png","svg","pdf"].includes(e)))&&o("formats","Choose PNG, SVG or PDF."),(!n.variants.length||n.variants.length>24||n.inputs.length>64||n.choices.length>4||n.recipes.length>64)&&o("limits","Use up to 24 artboards, 64 inputs and four choices.");let u=new Map(n.variants.map(e=>[e.id,e]));(u.size!==n.variants.length||!u.has(n.defaultVariant))&&o("artboards","Choose a valid default artboard and unique artboard ids.");for(let e of n.variants)(!/^[a-zA-Z0-9][a-zA-Z0-9_-]{0,63}$/.test(e.id)||![e.width,e.height].every(i=>Number.isFinite(i)&&i>=1&&i<=16384))&&o("artboard-size","Artboards need a size between 1 and 16384 pixels."),(e.boxes.length>1e3||new Set(e.boxes.map(i=>i.id)).size!==e.boxes.length)&&o("layers","Layers need unique, permanent ids.");let g=new Map(n.inputs.map(e=>[e.input.id,e])),r=new Set(n.choices.map(e=>e.inputId));(g.size!==n.inputs.length||r.size!==n.choices.length)&&o("duplicate-id","Each input and choice needs a unique id.");let a=new Map,c=new Map,x=new Map,f=(e,i,t)=>{if(!v(R,e.property)){o("property","This property cannot be made editable.",i,e.layerId);return}u.get(e.variantId)?.boxes.find(d=>d.id===e.layerId)||o("missing-layer","The linked object was removed. Relink this input.",i,e.layerId),t&&!R[e.property].includes(t)&&o("type","The input type does not match its property.",i,e.layerId);let h=a.get(w(e));h&&h!==i&&o("ownership","Two controls change the same property. Choose one owner.",i,e.layerId),a.set(w(e),i)};for(let e of n.inputs){let i=e.input;if((!O.test(i.id)||z.has(i.id)||M.has(i.id)||p.includes(i.id)||i.id.startsWith("__"))&&o("input-id","Use a unique input id that is not a reserved setting.",i.id),["text","longtext","number","select","color","asset","vector",...n.sourceTool?["boolean","url","date","time","datetime-local"]:[]].includes(i.type)||o("input-type","This input type is not supported in a locked tool.",i.id),i.type==="select"&&(!i.options?.length||i.options.some(t=>!t.value||!t.label)||new Set(i.options.map(t=>t.value)).size!==i.options.length)&&o("options","Give each option a label and a unique value.",i.id),i.type==="number"&&(![i.min,i.max,i.step,i.default].every(t=>typeof t=="number"&&Number.isFinite(t))||Number(i.min)>Number(i.max)||Number(i.step)<=0)&&o("range","Set a finite minimum, maximum, default and positive step.",i.id),i.type==="asset"&&i.assetType!=="image"&&o("media","Editable images must use the image asset type.",i.id),i.type==="vector"){let t=i.fields;(!t?.length||t.length>3||new Set(t.map(s=>s.id)).size!==t.length||t.some(s=>!["x","y","zoom"].includes(s.id)||![s.min,s.max,s.step].every(Number.isFinite)||s.min>s.max||s.step<=0||s.min<(s.id==="zoom"?1:0)||s.max>(s.id==="zoom"?1e3:100)))&&o("framing-range","Set valid X/Y percentages and a bounded zoom range.",i.id)}!e.targets.length&&!n.sourceTool?.inputs[i.id]&&!r.has(i.id)&&!n.recipes.some(t=>t.parts.some(s=>"inputId"in s&&s.inputId===i.id))&&o("unlinked","Link this input to an object.",i.id),e.common&&(e.common.key==="headshot"?i.type!=="asset":!["text","longtext"].includes(i.type))&&o("common-type","Use an image for a headshot and text for other common fields.",i.id),e.image&&(i.type!=="asset"||[e.image.minWidth,e.image.minHeight].some(t=>t!==void 0&&(!Number.isInteger(t)||t<1||t>32768))||e.image.formats&&(!e.image.formats.length||e.image.formats.some(t=>!["png","jpeg","webp","avif","svg"].includes(t))))&&o("image-rule","Choose image formats and positive minimum pixel dimensions.",i.id),e.common?.source==="profile"&&(!["person","recipient","presenter"].includes(e.common.subject)||!["firstname","lastname","email"].includes(e.common.key))&&o("profile","Profile prefill is only available for a person\u2019s name or email.",i.id);for(let t of e.targets)f(t,i.id,i.type),t.text&&(!(t.text.min>0)||!Number.isFinite(t.text.max)||t.text.max<t.text.min)&&o("text-fit","Review this object\u2019s font-size range.",i.id,t.layerId);e.text&&(!(e.text.min>0)||!Number.isFinite(e.text.max)||e.text.min>e.text.max||e.text.maxLines!==void 0&&(!Number.isInteger(e.text.maxLines)||e.text.maxLines<1))&&o("text-fit","Set a valid font-size range and line limit.",i.id)}if(n.sourceTool){let e=Object.entries(n.sourceTool.inputs);(!n.sourceTool.id||!n.sourceTool.version||e.length!==n.inputs.length||new Set(e.map(([,i])=>i)).size!==e.length||e.some(([i,t])=>!g.has(i)||!t||z.has(t)||t.startsWith("__")))&&o("source-inputs","Choose distinct inputs from the source tool.")}let m=1,b=0;for(let e of n.choices){let i=g.get(e.inputId);(i?.input.type!=="select"||i.targets.length)&&o("choice","A choice needs its own select input.",e.inputId),m*=e.options.length,(!e.options.length||new Set(e.options.map(s=>s.value)).size!==e.options.length)&&o("choice-options","Give each choice option a unique value.",e.inputId),e.options.some(s=>s.variantId)&&b++;let t=new Set;for(let s of e.options){i?.input.options?.some(d=>d.value===s.value)||o("choice-option","Declare each choice option in the input.",e.inputId),s.variantId&&!u.has(s.variantId)&&o("choice-artboard","Choose an existing artboard.",e.inputId);for(let d of new Set([...s.fixedInputs??[],...Object.keys(s.defaults??{})])){let y=g.get(d);(!y||r.has(d))&&o("choice-field","Choose an existing content input.",e.inputId);let V=x.get(d);if(V&&V!==e.inputId&&o("ownership","Two choices set the same input. Choose one owner.",d),x.set(d,e.inputId),y&&v(s.defaults||{},d))for(let I of D({...T(n),choices:[],inputs:[y]},{[d]:s.defaults[d]}))l.push(I)}let h=new Set;for(let d of s.writes){let y=w(d),V=c.get(y);V&&V!==e.inputId&&o("ownership","Two choices control the same property.",e.inputId,d.layerId),c.set(y,e.inputId),h.has(y)&&o("choice-write","An option cannot write the same property twice.",e.inputId,d.layerId),h.add(y);let I=a.get(y);I&&I!==e.inputId&&!(s.fixedInputs??[]).includes(I)&&o("ownership","This option and a reader input both control this property.",I,d.layerId),I||f(d,e.inputId),t.add(y)}}}(m>128||b>1)&&o("choice-limits","Use one artboard choice and up to 128 option combinations.");for(let e of n.recipes){(e.target.property!=="text"||!e.parts.length||e.parts.length>16)&&o("recipe","Joined text needs a text target and up to 16 parts."),f(e.target,`recipe:${w(e.target)}`,"text");for(let i of e.parts)"inputId"in i&&!["text","longtext"].includes(g.get(i.inputId)?.input.type??"")&&o("recipe-input","Joined text can use only declared text inputs.",i.inputId)}for(let e of D(T(n),{},!0))l.push(e);return l}function k(n,p){let l=n.defaultVariant,o=new Set,u={},g=[];for(let r of n.choices){let a=v(p,r.inputId)?p[r.inputId]:n.inputs.find(x=>x.input.id===r.inputId)?.input.default,c=r.options.find(x=>x.value===a);if(c){c.variantId&&(l=c.variantId);for(let x of c.fixedInputs??[])o.add(x);Object.assign(u,c.defaults),g.push(...c.writes)}}return{variantId:l,fixed:o,defaults:u,writes:g}}function D(n,p,l=!1){let o=[],u=k(n,p);for(let g of n.inputs){let r=g.input;if(u.fixed.has(r.id))continue;let a=v(p,r.id)?p[r.id]:v(u.defaults,r.id)?u.defaults[r.id]:r.default,c=f=>{o.push({code:"value",message:`${r.label||r.id}: ${f}`,inputId:r.id})};if(a==null||a===""){l&&r.required&&c("add a value.");continue}if(["text","longtext","url","date","time","datetime-local"].includes(r.type))typeof a!="string"?c("enter text."):r.maxLength!==void 0&&a.length>r.maxLength&&c(`use up to ${r.maxLength} characters. The full value has been kept.`);else if(r.type==="boolean")typeof a!="boolean"&&c("choose on or off.");else if(r.type==="number"){let f=Number(a);(!Number.isFinite(f)||f<Number(r.min)||f>Number(r.max)||Math.abs((f-Number(r.min))/Number(r.step)-Math.round((f-Number(r.min))/Number(r.step)))>1e-7)&&c(`use ${r.min} to ${r.max}, in steps of ${r.step}.`)}else if(r.type==="select"&&!r.options?.some(f=>f.value===a))c("choose an approved option.");else if(r.type==="color"&&(typeof a!="string"||!/^#[\da-f]{3,8}$/i.test(a)))c("choose a valid colour.");else if(r.type==="asset"&&typeof a=="object"&&"type"in a&&(!["raster","vector"].includes(String(a.type))||"format"in a&&["gif","apng","html"].includes(String(a.format))))c("choose a still image.");else if(r.type==="vector"){let f=r.fields;if(!a||typeof a!="object"||!f?.length)c("set valid image positioning.");else for(let m of f){let b=Number(a[m.id]??r.default?.[m.id]);(!Number.isFinite(b)||b<m.min||b>m.max||m.step&&Math.abs((b-m.min)/m.step-Math.round((b-m.min)/m.step))>1e-7)&&c(`${m.id} must be between ${m.min} and ${m.max}, using its declared step.`)}}let x=typeof a=="object"&&a&&"id"in a?String(a.id):String(a);g.approved?.length&&!g.approved.includes(x)&&c("choose a designer-approved value.")}return o}function _(n,p){let l=T(n),o=k(l,p),u=n.variants.find(t=>t.id===o.variantId);if(!u)throw new Error("The selected artboard is unavailable.");let g=JSON.parse(JSON.stringify(u)),r={},a={},c={},x={},f={},m={},b=D(l,p,!0),e=new Set(b.map(t=>t.inputId)),i=(t,s)=>{if(t.variantId!==g.id)return;let h=g.boxes.find(d=>d.id===t.layerId);h&&v(R,t.property)&&(h[t.property]=s,t.property==="text"&&(h.plainText=!0))};for(let t of n.inputs){let s=t.input,h=!e.has(s.id)&&v(p,s.id)?p[s.id]:v(o.defaults,s.id)?o.defaults[s.id]:s.default;if(r[s.id]=o.fixed.has(s.id)?o.defaults[s.id]??s.default:h,!o.fixed.has(s.id))for(let d of t.targets)i(d,h),d.variantId===g.id&&(c[d.layerId]??=s.id,d.property==="imageFraming"&&(x[d.layerId]=s.id),d.property==="image"&&t.image&&(m[d.layerId]=t.image),d.property==="text"&&t.text?.sharedSize&&(f[d.layerId]=s.id),t.text&&d.property==="text"&&(a[d.layerId]=d.text||t.text))}for(let t of n.recipes)if(i(t.target,t.parts.map(s=>"literal"in s?s.literal:String(r[s.inputId]??"")).join("")),t.target.variantId===g.id){let s=t.parts.find(h=>"inputId"in h);s&&"inputId"in s&&(c[t.target.layerId]=s.inputId),t.text&&(a[t.target.layerId]=t.text)}for(let t of o.writes)i(t,t.value);for(let t of n.inputs)if(t.input.type==="number"&&!o.fixed.has(t.input.id))for(let s of t.targets)s.variantId===g.id&&s.property==="fontSize"&&a[s.layerId]&&(a[s.layerId]={...a[s.layerId],min:Number(t.input.min),max:Number(t.input.max)});for(let t of g.boxes){let s=a[String(t.id)];t.fitText=!1,s&&(t.fontSize=Math.min(s.max,Math.max(s.min,Number(t.fontSize)||s.max)))}return{variant:g,values:r,findings:b,textRules:a,inputMap:c,framingMap:x,fitGroups:f,imageRules:m}}return C(L);})();
