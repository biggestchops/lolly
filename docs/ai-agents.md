...
For a known recipe, do not enumerate the whole catalog or repeat validation as a separate call. For a revision, send stable layer patches or changed inputs instead of the whole document. A result is complete when its declared checks pass. Ask for human review only when the task names a requirement this instance cannot measure, such as physical print appearance or subjective approval. Otherwise report the result and continue.
